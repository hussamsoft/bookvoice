...
- **Cloudflare Tunnel moved into the shared server path.** `serve_bookvoice.py` (and therefore `Start-BookVoice-Server.bat` and the WinUI desktop shell, which forwards the same flags) supports `--tunnel`, `--tunnel-name`, `--tunnel-hostname`, and `--tunnel-token` with the semantics of the old launcher: settings persist in the runtime folder so a configured tunnel comes back on later launches of any entry point, the public URL is printed, published to the state files, and shown in Settings, and a dashboard-managed tunnel warns when its fixed port is not pinned. The tunnel URL also relaxes the backend's origin/cookie policy exactly as before via `apply_tunnel_env`.
- **`BookVoice.exe --register-bookvoice`** records a per-user `.bookvoice` file association (`HKCU\Software\Classes`, no admin) so double-clicked prepared books open in the portable desktop app; `--unregister-bookvoice` removes it. This closes the portable-payload gap left by the retired MSI-era launcher.
- **A native Windows desktop shell built on WinUI 3** (`desktop/BookVoice.App`, shipped as `dist/desktop/BookVoice.exe`). The self-contained, unpackaged WinUI app starts the same bundled backend the browser-mode launcher did — packaged worker Python running `serve_bookvoice.py` — then renders the app in WebView2 inside a normal desktop window (Mica backdrop where supported, remembered size and position, single-instance: launching it again raises the existing window). A Fluent splash tracks startup with the same phases the old shell reported; failures show the message with the server-log tail and Try again / Open log folder / Close actions. Closing the window stops the backend process tree, and the watchdog still restarts a backend that dies or stops answering `/api/health` (five restarts, as before). `.bookvoice` paths on the command line import and open after readiness, including handed over from a second launch. The WebView2 profile and `--disable-gpu` default (opt back in with `BOOKVOICE_ENABLE_GPU=1`) match the old pywebview shell, so user data and GPU behavior carry over; `build.py` publishes the shell into `dist/desktop/` and validation requires it.
- **`serve_bookvoice.py` — a headless server console shared by every entry point.** It reuses `launch.py`'s resolution, validation, voice-library recovery, port scan, and env block (so the desktop shell, the bat, and a manual run all agree on runtime layout by construction), then runs the packaged uvicorn backend with a startup watchdog and reprints the URLs after each recovery. It publishes `server-state.json` (starting/ready/error with the chosen port) so the desktop shell reads the selected port instead of re-implementing it, and it can import a `.bookvoice` archive passed on the command line.
...
- **App windows now launch at about 65% of the monitor's work area, centered** — the WinUI 3 desktop shell and the Electron shell both compute the size from the primary display's work area and floor it at the smallest size every element genuinely compresses to (780×560; the reader toolbar wraps at ≤720 px and panels scroll below that). Saved window placement still wins on later launches, and the maximized-restore behavior is unchanged. The WebView2 host background and the Electron `backgroundColor` now match the dark stage color so first paint never flashes the old paper theme, and the legacy `launch.py` splash/error pages were recolored onto the Aurora palette (progress bar included).
- `build.py` no longer builds the PyInstaller `Launcher.exe` into the payload — the WinUI desktop shell replaces it (`--legacy-launcher` rebuilds the old way, and the MSI flow that depends on `Launcher.exe` will need rework before the next release). `dist/RUN.md` documents the new entry points.
...
- The pdf.js/react-pdf core is now a shared lazy chunk used by both the production viewer and the new reader, instead of being duplicated in the viewer's bundle.

### Removed

### Fixed

- **Watchdog restarts now re-prove the port instead of re-binding blindly.** Both watchdogs (`launch.py` desktop, `serve_bookvoice.py` headless) used to restart uvicorn on the same port unconditionally — if another app took it while the backend was down, every restart died on the bind. Scan mode now keeps the old port only when it still binds free and otherwise moves via the same scan (pinned ports still never move), and a restart that lands elsewhere re-publishes `server-state.json`, `server-access.json`, and the sticky port while the desktop shell reloads the window on the fresh URL. The dev launcher gets the same scan-mode steal-retry the packaged paths already had. Dev tooling stops assuming 8000: `simulate_app.py`/`smoke_api.py`/`smoke_cloning_api.py` take a free loopback port (`simulate_app` keeps `--port` as a preference, not an assertion), `setup_linux.sh` gains `--port N`, the dead `PORT=` key leaves `backend/.env.example`, `frontend/.env.example` points at same-origin `/api`, and `vite.config.js` gains an `/api` dev proxy (`BOOKVOICE_DEV_API` overrides the 8000 default).

### Test results

- Frontend unit: 391/391 passing (66 files) — includes narration coverage: streaming play through the transport, cancel-on-navigate, prepared-audio resume parking, mute, and the Space shortcut
- Frontend lint: 0 diagnostics
- Backend pytest: 436 passed (24 subtests)
...
- Build: initial entry 309.28 KiB against the 350 KiB budget (the new `Reader` chunk is lazy-loaded at 26.8 KiB); `build.py` validation passes with the desktop shell required in the payload
- Live verification on the fresh `dist/`: `desktop\BookVoice.exe` starts the backend, reaches ready, renders the UI in WebView2, and closing it stops the process tree and releases the port; `Start-BookVoice-Server.bat` binds the LAN, prints the device URLs, and serves `/api/health`; `scripts/smoke_launch.py --skip-server` passes on the payload
- Live port/tunnel verification: pinned port → sticky reuse → busy-port scan each resolve consistently with `server-port.json` tracking the last ready port; a real quick tunnel published a public URL that served `/api/health` through Cloudflare; `GET /api/server/addresses` reports the published access file (unit-tested against missing/corrupt files too)
- Mobile audits at 390 px/844 px: measured touch-target and toolbar-overflow checks pass after the fixes; contrast audit reports zero violations; 320 px reflow has no horizontal overflow
- Redesign verified in a live browser at the 65% launch scale and below — 1248×702 (65% of 1080p), 888×560 (small-laptop floor), and 390×844 (phone) — in both light and dark modes across the Reader, Voice Studio, and Settings surfaces; `WinUI 3 shell (Release)` and `node --check` on the Electron main both compile clean after the window-sizing change
